...
export default function App({navigation}) {
  ...
  if (photo) {
    const savePhoto = async () => {
      if (photo) {
        try {
          // Convert the photo object to a string
          await AsyncStorage.setItem('@profile_photo', JSON.stringify(photo));
          navigation.navigate('CreateAccount1'); // Go back to the SignUp screen
        } catch (error) {
